...
def generate_random_name(name_prefix):
    # পাওয়ার সংখ্যার তালিকা
    powers = ['⁰', '¹', '²', '³', '⁴', '⁵', '⁶', '⁷', '⁸', '⁹']
    # র্যান্ডম ৬টি পাওয়ার নির্বাচন
    random_powers = ''.join(random.choice(powers) for _ in range(6))
    return name_prefix + random_powers

def generate_custom_password():
    characters = string.ascii_letters + string.digits
    random_part = ''.join(random.choice(characters) for _ in range(9)).upper()
    return f"TANVIR-GAMING-{random_part}"
